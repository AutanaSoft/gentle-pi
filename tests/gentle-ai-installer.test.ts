...
// v2.8.0 digests pinned from the published release: archive digests verified
...
const EXPECTED_ASSETS = {
	"darwin/amd64": { name: "gentle-ai_2.8.0_darwin_amd64.tar.gz", sha256: "9c735e51916c31135926e62b21ac990216d810907a73894f68ec47bb0648d22d", binarySha256: "3880084f118533102a02eb0656a709db9817d330f82de8f533868667e0ac449d" },
	"darwin/arm64": { name: "gentle-ai_2.8.0_darwin_arm64.tar.gz", sha256: "b62d01aadba484e79869e11878d47e81cd692ebce93a5de454d0889db65fac54", binarySha256: "fed81b2b3dbec02ab5f06ce73364b43d28c532231813a01e2015d585f08cbb2e" },
	"linux/amd64": { name: "gentle-ai_2.8.0_linux_amd64.tar.gz", sha256: "56378fd47cfee127d5982398e79248194d3c7624d268efda9823a6b061a018b7", binarySha256: "df19c4ac20cfdfc0cc0c98c75cda42106340ca9966009f5dfb61575c1ada27c8" },
	"linux/arm64": { name: "gentle-ai_2.8.0_linux_arm64.tar.gz", sha256: "41726bffee269206e5fe96d7bd900164d01f478d8916bdfba446a7991903dfc1", binarySha256: "a93fff44ea583546d37ce833cc2a0f14afec118ae4209c305cafcadb91f67bac" },
} as const;
...
test("release mapping selects only the supported official v2.8.0 assets and pinned digests", () => {
	assert.deepEqual(
	...
	assert.equal(resolveGentleAiReleaseAsset("linux", "x64").name, "gentle-ai_2.8.0_linux_amd64.tar.gz");
	assert.equal(resolveGentleAiReleaseAsset("darwin", "arm64").name, "gentle-ai_2.8.0_darwin_arm64.tar.gz");
	for (const asset of Object.values(GENTLE_AI_RELEASE_ASSETS)) {
		assert.match(asset.url, /^https:\/\/github\.com\/Gentleman-Programming\/gentle-ai\/releases\/download\/v2\.8\.0\//);
	}
});
...
test("raw release assets are admitted only under a prerelease pin", () => {
	...
	assert.throws(() => gentleAiAssetForm("gentle-ai_2.8.0_linux_amd64", "2.8.0"), /only admitted for a prerelease pin/);
...
test("release digests are all-or-none and install fails closed while any digest is pending", async () => {
	...
	if (pending.length === digests.length) {
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.8.0", "gentle-ai")), false);
	}
});
...
function windowsGoFixture(fixtureOptions: WindowsGoFixtureOptions = {}) {
	...
	const metadata = [
		...
		`\tmod\tgithub.com/gentleman-programming/gentle-ai/v2\tv2.8.0\t${GENTLE_AI_WINDOWS_SOURCE_MODULE_CHECKSUM}`,
	...
	const run = async (file: string, arguments_: string[], options: WindowsGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: fixtureOptions.reportedVersion ?? "gentle-ai 2.8.0\n", stderr: "" };
...
test("win32 x64 and arm64 install the exact Go SumDB source tag without archive lookup", async () => {
	for (const [arch, goArchitecture] of [["x64", "amd64"], ["arm64", "arm64"]] as const) {
		...
		assert.deepEqual(fixture.calls.filter((call) => call.file === goPath).map((call) => call.arguments_.slice(0, 2)), [
			["version"], ["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.8.0"], ["version", "-m"],
		]);
	}
});
...
test("Windows source install reports missing or too-old Go without publishing a binary", async () => {
	for (const fixtureOptions of [
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.8.0", "gentle-ai.exe")), false);
...
test("Windows source install cleans staging after Go failure or wrong built version", async () => {
	for (const fixtureOptions of [
		...
		const runtimeDirectory = join(packageRoot, ".gentle-ai", "v2.8.0");
...
test("Windows source installs reuse only a fully verified package-local binary", async () => {
	...
	await writeFile(join(packageRoot, ".gentle-ai", "v2.8.0", "integrity.json"), "{}\n");
...
async function hardenedWindowsGoFixture(packageRoot: string, fixtureOptions: HardenedGoFixtureOptions = {}) {
	...
	const run = async (file: string, arguments_: string[], options: HardenedGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: "gentle-ai 2.8.0\n", stderr: "" };
		throw new Error(`unexpected command: ${file} ${arguments_.join(" ")}`);
	};
	return {
		calls,
...
test("Windows source installation resolves one validated Go executable and seals hostile Go environment", async () => {
	...
	assert.deepEqual(goCalls.map((call) => call.arguments_.slice(0, 2)), [
		["version"],
		["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.8.0"],
		["version", "-m"],
	]);
...
test("Windows source manifest binds verified Go metadata and architecture", async () => {
	...
	const manifest = JSON.parse(await readFile(join(packageRoot, ".gentle-ai", "v2.8.0", "integrity.json"), "utf8")) as Record<string, string>;
	assert.equal(result.installed, true);
...
test("Windows source installation treats a fresh ownerless lock as active", async () => {
	...
	const lockPath = join(packageRoot, ".gentle-ai", ".v2.8.0.install.lock");
...
test("Windows source installation preserves a lock whose owner nonce changed before release", async () => {
	...
	const lockOwnerPath = join(packageRoot, ".gentle-ai", ".v2.8.0.install.lock", "owner.json");
	const fixture = await hardenedWindowsGoFixture(packageRoot);
...
function tombstonePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.8.0.install.tombstone-${nonce}`);
}
...
async function tombstones(packageRoot: string): Promise<string[]> {
	...
	return existsSync(runtimeRoot)
		? (await readdir(runtimeRoot)).filter((entry) => entry.startsWith(".v2.8.0.install.tombstone-"))
		: [];
}

function backupBundlePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.8.0.backup-${nonce}`);
}
...
test("Windows source acquisition fails closed when a tombstone appears after its first scan", async () => {
	...
	assert.equal(existsSync(join(packageRoot, ".gentle-ai", ".v2.8.0.install.lock")), false);
	assert.equal(fixture.calls.some((call) => call.arguments_[0] === "install"), false);
});
...
test("Windows source recovers a valid backup after a crash between publication renames before any build", async () => {
	...
	const live = join(packageRoot, ".gentle-ai", "v2.8.0");
...
test("Windows source publication rolls back a prior bundle when final directory swap fails", async () => {
	...
	const versionDirectory = join(packageRoot, ".gentle-ai", "v2.8.0");
...
test("Darwin/Linux signed bundles retain their four-field manifest and reusable compatibility", async () => {
	...
	const manifestPath = join(packageRoot, ".gentle-ai", "v2.8.0", "integrity.json");
...
test("a pinned asset falls back to its gentle-pi mirror only on download failure, and a stable pin declares no mirror", async () => {
	...
	const baseAsset = { name: "gentle-ai_2.8.0_linux_amd64.tar.gz", sha256: createHash("sha256").update(payload).digest("hex"), binarySha256: createHash("sha256").update(binary).digest("hex"), url: "https://example.invalid/upstream", mirrorUrl: "https://example.invalid/mirror", executable: "gentle-ai" };
...
test("installer promotes only the expected regular executable with executable POSIX mode", async () => {
	...
	const binary = join(packageRoot, ".gentle-ai", "v2.8.0", "gentle-ai");
